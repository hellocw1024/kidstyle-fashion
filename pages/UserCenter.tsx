...
const UserCenter: React.FC<Props> = ({ user, onLogout, onUpdateUser, resources, rechargeRequests, onAddRechargeRequest, onRemoveResource, onToggleFavorite }) => {
  const [activeTab, setActiveTab] = useState<'RESOURCES' | 'RECHARGE' | 'HISTORY' | 'SETTINGS'>('RESOURCES');
  ...
  const filteredAndSearchedResources = resources.filter(res => {
    ...
    if (searchQuery) {
      ...
      const matchCategory = res.category.toLowerCase().includes(query);
      const matchTags = res.tags.some(t => t.toLowerCase().includes(query));
      const matchDate = res.date.includes(query);
      if (!matchCategory && !matchTags && !matchDate) return false;
    }

    // 高级筛选
    if (filters.category && res.category !== filters.category) return false;
    if (filters.season && res.season !== filters.season) return false;
  ...
  console.log('  - 资源列表:', resources.map(r => ({ type: r.type, category: r.category, id: r.id })));
  ...
  const handleBatchDownload = () => {
    selectedIds.forEach(id => {
      ...
      if (resource) {
        setTimeout(() => {
          handleDownload(resource.url, `小红衣_${resource.category}_${resource.date}.png`);
    ...
    try {
      ...
      for (const resource of resources) {
        try {
          ...
          // 创建文件名
          const filename = `小红衣_${resource.category}_${resource.date}_${resource.id}.png`;
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <main className="flex-1 bg-white rounded-3xl shadow-sm border p-8">
                        ...
                        <div key={res.id} className={`group relative bg-gray-50 rounded-2xl overflow-hidden transition-all ${selectionMode ? 'border-2' : 'border border-gray-100'} ${selectionMode && selectedIds.includes(res.id) ? 'border-blue-500 shadow-lg' : ''}`}>
                          ...
                          <div className="p-4">
                             <div className="flex items-center justify-between mb-2">
                               <span className="text-xs font-bold text-rose-500 bg-rose-50 px-2 py-0.5 rounded uppercase">{res.category}</span>
                          ...
                          <div className="absolute top-2 right-2 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() => handleDownload(res.url, `小红衣_${res.category}_${res.date}.png`)}
           ...
           {activeTab === 'HISTORY' && (
             <div className="space-y-6">
                <h3 className="text-xl font-bold">配额消耗明细</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead className="border-b bg-gray-50">
                      <tr>
                        <th className="px-4 py-3 font-bold text-gray-600">时间</th>
                        <th className="px-4 py-3 font-bold text-gray-600">内容</th>
                        <th className="px-4 py-3 font-bold text-gray-600">数量</th>
                        <th className="px-4 py-3 font-bold text-gray-600">状态</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                       {resources.filter(r => r.type === 'GENERATE').map(res => (
                         <tr key={res.id} className="hover:bg-gray-50/50">
                           ...
                           <td className="px-4 py-4 font-medium text-gray-700">生成 - {res.category} ({res.tags.includes('4K') ? '4K' : res.tags.includes('2K') ? '2K' : '1K'})</td>
          ...
          <div className="relative bg-white rounded-[2.5rem] w-full max-w-xl p-8 shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                {RECHARGE_OPTIONS.map((opt, i) => (
                  <button 
                    ...
                    className={`p-4 rounded-2xl border-2 transition-all text-center relative ${
                      selectedAmount === opt ? 'border-rose-500 bg-rose-50' : 'border-gray-100 hover:border-rose-200'
                    }`}
                  >
             ...
             <div className="bg-gray-50 p-6 rounded-3xl border border-dashed border-gray-200 mb-8 flex flex-col items-center">
                ...
                <div 
                  ...
                  className={`w-full py-4 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all ${
                    rechargeScreenshot ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-rose-400'
                  }`}
                >
                   {rechargeScreenshot ? (
